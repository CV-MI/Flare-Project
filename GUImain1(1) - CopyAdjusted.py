...
def open_results_visualizer_window():
    ...
    def update_images(index):
        ...
        if segmentation_smoke_path is not None and os.path.exists(segmentation_smoke_path):
            segmentation_smoke = load_image(segmentation_smoke_path, max_size=(400, 300))
        else:
            # If no segmentation image is found, create a black image of the same size
            segmentation_smoke = create_black_image(400, 300)
            # segmentation_smoke_path = find_matching_image(segmentation_visualized_path2, os.path.splitext(all_images[index])[0])
    ...
    with open(os.path.join(raw_detection_labels_path, yolo_labels_filename), "w") as output_file:
        ...
        for result in results:
            ...
            if boxes.numel() == 0:
                ...
                img_height, img_width = image.size
                empty_img = np.zeros((img_width, img_height, 3), dtype=np.uint8)
                empty_flame_img_filename = os.path.splitext(os.path.basename(image_path))[0] + "_flame_empty.png"
                empty_smoke_img_filename = os.path.splitext(os.path.basename(image_path))[0] + "_smoke_empty.png"
                empty_img_flame_path = os.path.join(segmented_flame_path, empty_flame_img_filename)
                empty_img_smoke_path = os.path.join(segmented_smoke_path, empty_smoke_img_filename)

                # If no flame detected, save the black image for flame
                if not flame_detected:
                    cv2.imwrite(empty_img_flame_path, empty_img)

                    # Also save an empty .txt file for flame segmentation labels
                    empty_flame_seg_filename = os.path.splitext(os.path.basename(image_path))[0] + "_flame_segmentation_empty.txt"
                    empty_flame_seg_path = os.path.join(raw_segmentation_labels_path1, empty_flame_seg_filename)
                    open(empty_flame_seg_path, 'w').close()

                # If no smoke detected, save the black image for smoke
                if not smoke_detected:
                    cv2.imwrite(empty_img_smoke_path, empty_img)

                    # Also save an empty .txt file for smoke segmentation labels
                    empty_smoke_seg_filename = os.path.splitext(os.path.basename(image_path))[0] + "_smoke_segmentation_empty.txt"
                    empty_smoke_seg_path = os.path.join(raw_segmentation_labels_path2, empty_smoke_seg_filename)
                    open(empty_smoke_seg_path, 'w').close()
